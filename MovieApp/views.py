...
def add_movie_view(request):
    ...
    if request.method=="POST":
        ...
        if formObj.is_valid():
            ...
            print(formObj.cleaned_data['ratings'])
            formObj.save()	#auto-commit
            return index_view(request)
    return render(request, 'MovieApp/addmovie.html',{'form1':formObj})

def list_movie_view(request):
    movies_list=Movies.objects.all().order_by('-ratings') #(-)desc-order
    return render(request,'MovieApp/listview.html',{'movies_list':movies_list});